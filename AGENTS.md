...
- Use the supplied 27-screen Figma export as the visual source of truth for this horizontal porcelain game; visible UI copy is natural English.
- A-05 (artifact introduction) is mandatory after A-04 (level map).
- C-A-02 uses mock bidding only: raising a bid shows confirmation and stays on the detail screen; no payment integration.
- Keep motion light and natural: short fades, gentle press feedback, restrained success/error animation, smooth progress, and quiet ceramic/wood sound cues.
- The launch logo must use the supplied PNG layers (brush ring, 瓷/艺/坊, vase, and seal), with a subtle wind-like staggered sway; do not redraw these assets in CSS.
- Use the supplied audio-only “Moonlit Shadows” track as looping BGM, louder on launch/login screens and quieter elsewhere. The launch screen waits for the player to enter and must not advance automatically. Use the supplied ceramic-collision MP3 for button clicks.
- Environmental motion should be visibly rhythmic while remaining refined: eight evenly staggered dust motes on an eight-second cycle, an eight-second ambient-light sweep, a progress-linked glaze highlight, and a 3.2-second finished-porcelain golden breathing cycle.
- Selection feedback follows a museum-display tone: quiet default state, modest lift and soft detail label on hover, then a restrained gold edge and one porcelain-sheen pass on confirmation. Avoid large flashes, dense particles, or arcade-style victory effects. Page changes should open from the center like a painted scroll, and crafting completion may use only a small halo and five local motes.
- Treat A-07 shaping, A-08 carving, and A-09 glazing as stages of one shared pottery interaction system. Reuse the supplied GLB, camera, scale, center, turntable, lighting, and retained rotation angle across all three; change only tool/hand/effect/progress layers. Carving uses lightweight traces and glazing progressively reduces material roughness rather than replacing the model.
- On the shaping screen, render the supplied `public/assets/ceramic/models/ceramic-vase.glb` as the central Three.js object. Horizontal pointer delta rotates the real model around its vertical axis with short, weighty inertia; the model stays centered while the 3D pottery wheel follows subtly. Use warm studio key light, cool fill, gold rim light, soft shadows, and tool-dependent ceramic material response. Show a one-time drag coach. The selected top tool alone is enlarged, pale/white, and gold-edged; inactive tools stay smaller and brown while their icons remain clearly visible. Next stays visually subdued until the craft threshold is reached, after which a brief Step Complete treatment unlocks it.
- The A-03 puzzle screen should feel actively staged: puzzle fragments float independently on a slow regular cadence, the vase receives a moving glaze highlight, and its original Introduction and Start making buttons own tightly fitted hover, press, and confirmation states.
